...
if not FLAGS.pre_training:
    train_set, valid_set, voc, dic_inv = int_processor.load_train_dev(FLAGS.scope_detection, FLAGS.event_detection,
                                                                      FLAGS.training_lang, checkpoint_dir)
    vocsize = len(voc['w2idxs'])
    tag_voc_size = len(voc['t2idxs']) if FLAGS.POS_emb == 1 else len(voc['tuni2idxs'])
else:
    train_set, valid_set, dic_inv, pre_emb_w, pre_emb_t = ext_processor.load_train_dev(FLAGS.scope_detection,
                                                                                       FLAGS.event_detection,
                                                                                       FLAGS.training_lang,
                                                                                       FLAGS.embedding_dim,
                                                                                       FLAGS.POS_emb)
...
def feeder(_bilstm, lex, cue, tags, _y, train=True):
    ...
    Y = padding(np.asarray(map(lambda x: [1, 0] if x == 0 else [0, 1], _y)).astype('int32'), FLAGS.max_sent_length,
                0, False)
    ...
    feed_dict = {
        ...
        _bilstm.seq_len: np.asarray([len(lex)]),
        _bilstm.mask: _mask}
    if tags != []:
    ...
    with sess.as_default():
        ...
        for e in xrange(FLAGS.num_epochs):
            ...
            for i in xrange(len(train_lex)):
                if FLAGS.POS_emb in [1, 2]:
                    loss_train = feeder(bi_lstm, train_lex[i], train_cue[i],
                                        train_tags[i] if FLAGS.POS_emb == 1 else train_tags_uni[i], train_y[i])
                else:
                    loss_train = feeder(bi_lstm, train_lex[i], train_cue[i], [], train_y[i])
                # Calculating batch accuracy
                train_step_loss.append(loss_train)
                print('[learning] epoch %i >> %2.2f%%' % (e, (i + 1) * 100. / len(train_lex)),
            ...
            for i in xrange(len(valid_lex)):
                if FLAGS.POS_emb in [1, 2]:
                    acc_dev, pred, Y_dev = feeder(bi_lstm, valid_lex[i], valid_cue[i],
                                                  valid_tags[i] if FLAGS.POS_emb == 1 else valid_tags_uni[i],
                                                  valid_y[i], train=False)
                else:
