...
from utils.tools import padding, unpickle_data
...
# ==================================================
# Model Parameters
tf.flags.DEFINE_string("test_set", 'NegNN/data/test/sherlock_cardboard.txt', "Path to the test filename (to use only in test mode")
tf.flags.DEFINE_string("checkpoint_dir", 'NegNN/runs/noPRE_21495660256/checkpoints', "Path to the directory where the last checkpoint is stored")
...
if not pre_training:
    ...
    test_lex, test_tags, test_tags_uni, test_cue, _, test_y = int_processor.load_test(test_files, voc, scope_dect,
                                                                                      event_dect, FLAGS.test_lang)
else:
    test_set, dic_inv, pre_emb_w, pre_emb_t = ext_processor.load_test(test_files, scope_dect, event_dect,
                                                                      FLAGS.test_lang, embedding_dim, POS_emb)
...
def feeder(_bilstm, lex, cue, tags, _y, train=True):
    ...
    Y = padding(numpy.asarray(list(map(lambda x: [1, 0] if x == 0 else [0, 1], _y))).astype('int32'), max_sent_length, 0,
                False)
...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        for i in xrange(len(test_lex)):
            if POS_emb in [1, 2]:
                acc_test, pred_test, Y_test = feeder(bi_lstm, test_lex[i], test_cue[i],
                                                     test_tags[i] if POS_emb == 1 else test_tags_uni[i], test_y[i],
                                                     train=False)
            else:
        ...
        recalls = []
        f1_scores = []
        _, report_tst, best_test, f1_pos = get_eval(preds_test, gold_test, precisions, recalls, f1_scores)
